...
class SubgraphEncoder(torch.nn.Module):
    def __init__(self, hidden_dim, num_vertices, num_subvertices, dropout=0.2, is_training=False):
    ...
    def forward(self, edge_index):
        x = self.conv1(edge_index)
        x = x.relu()
        x = self.conv2(edge_index)
...
class MultiviewGAE(torch.nn.Module):
    ...
    def forward(self, x_c, edge_index_g,edge_index_c):
        Z_gg = self.encoder_g( edge_index_g)
